...
test("paginates session messages backwards without breaking entry id alignment", () => {
  const context = {
    messages: Array.from({ length: 7 }, (_, index) => ({ role: "user", content: `message-${index}` })),
    entryIds: Array.from({ length: 7 }, (_, index) => `entry-${index}`),
    thinkingLevel: "medium",
    model: { provider: "test", modelId: "test-model" },
  };

  const latest = paginateSessionContext(context, 3);
  assert.deepEqual(latest.entryIds, ["entry-4", "entry-5", "entry-6"]);
  assert.deepEqual(latest.messages.map((message) => message.content), ["message-4", "message-5", "message-6"]);
  assert.deepEqual(latest.page, { hasMore: true, beforeEntryId: "entry-4", totalMessages: 7 });

  const older = paginateSessionContext(context, 3, latest.page.beforeEntryId);
  assert.deepEqual(older.entryIds, ["entry-1", "entry-2", "entry-3"]);
  assert.deepEqual(older.page, { hasMore: true, beforeEntryId: "entry-1", totalMessages: 7 });

  const oldest = paginateSessionContext(context, 3, older.page.beforeEntryId);
  assert.deepEqual(oldest.entryIds, ["entry-0"]);
  assert.deepEqual(oldest.page, { hasMore: false, beforeEntryId: null, totalMessages: 7 });
});
...
test("preserves hidden custom messages so the UI can render them collapsed", () => {
  ...
  assert.deepEqual(context.stats, {
    userMessages: 1,
    assistantMessages: 1,
    toolCalls: 0,
    toolResults: 0,
