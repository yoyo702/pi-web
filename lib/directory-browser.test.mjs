...
test("optionally hides dot directories", async () => {
  ...
  try {
    ...
    const { listDirectories } = await loadSubject();
    assert.deepEqual((await listDirectories(root)).map((entry) => entry.name), [".hidden", "visible"]);
    assert.deepEqual((await listDirectories(root, { hideHidden: true })).map((entry) => entry.name), ["visible"]);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
