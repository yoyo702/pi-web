...
test("sub-agent records stay under their tool call and a compaction shows once", () => {
  const items = claudeConversationItems([
    assistant("m1", 0, [{ type: "tool_use", id: "agent1", name: "Agent", input: { description: "Look" } }]),
    { type: "user", uuid: "s1", parentToolUseId: "agent1", message: { role: "user", content: [{ type: "text", text: "Look around" }] } },
    { type: "assistant", uuid: "s2", parentToolUseId: "agent1", message: { id: "ms", role: "assistant", content: [{ type: "text", text: "sub" }] } },
    { type: "system", subtype: "task_progress", tool_use_id: "agent1" },
    { type: "user", uuid: "r1", message: { role: "user", content: [{ type: "tool_result", tool_use_id: "agent1", content: [{ type: "text", text: "done" }] }] } },
    { type: "system", subtype: "compact_boundary", uuid: "b1" },
    { type: "user", uuid: "c1", message: { role: "user", content: "<local-command-stdout>Compacted </local-command-stdout>" } },
  ]);
  assert.deepEqual(items.map((item) => [item.id, item.kind, item.text ?? item.output]), [["agent1", "toolCall", "done"], ["b1", "notice", "Conversation compacted"]]);
});
