...
test("a Claude or Codex terminal is listed while it works or waits for approval, and completes when it waits for the next message", () => {
  const claude = (activity, overrides = {}) => terminal("c1", { provider: "claude", title: "Claude terminal", activity, ...overrides });
  const idle = computeRailActivity({ ...empty, terminals: [claude("waiting")] });
  assert.deepEqual(idle.items, []);
  const approval = computeRailActivity({ ...empty, terminals: [claude("approval")], previousRunning: idle.running });
  assert.deepEqual(approval.items.map((item) => [item.key, item.state]), [["terminal:c1", "approval"]]);
  const working = computeRailActivity({ ...empty, terminals: [claude("working")], previousRunning: approval.running });
  assert.deepEqual(working.items.map((item) => [item.key, item.state]), [["terminal:c1", "working"]]);
  const finished = computeRailActivity({ ...empty, terminals: [claude("waiting")], previousRunning: working.running });
  assert.deepEqual(finished.items.map((item) => [item.key, item.state, item.terminal.activity]), [["terminal:c1", "completed", "waiting"]]);
  // Unknown (a shell, or a CLI that has not reported yet): working while it runs.
  assert.equal(computeRailActivity({ ...empty, terminals: [claude(null)] }).items[0].state, "working");
  assert.deepEqual(["waiting", "working", "approval", null].map((activity) => terminalIsBusy(claude(activity))), [false, true, true, true]);
  assert.equal(terminalIsBusy(claude("working", { state: "ended" })), false);
});
...
test("lists busy Claude chats with their title and opens them as Claude targets", () => {
  const result = computeRailActivity({
    ...empty,
    claudeRuntimes: [
      { sessionId: "c1", cwd: "/repo/b", title: "Review the parser", owner: "chat", state: "approval" },
      { sessionId: "0123456789", cwd: "/repo/b", title: null, owner: "chat", state: "running" },
      { sessionId: "idle", cwd: "/repo/b", owner: "chat", state: "idle" },
    ],
  });
  assert.deepEqual(result.items.map((item) => [item.key, item.label, item.state]), [
    ["claude:c1", "Review the parser", "approval"],
    ["claude:0123456789", "Claude chat · 01234567", "working"],
  ]);
  assert.deepEqual(activityTarget(result.items[0]), { kind: "claude", id: "c1", cwd: "/repo/b", title: "Review the parser" });
  const done = computeRailActivity({ ...empty, claudeRuntimes: [{ sessionId: "c1", cwd: "/repo/b", owner: "chat", state: "idle" }], previousRunning: result.running, completed: result.completed });
  assert.deepEqual(done.items.map((item) => [item.key, item.state]), [["claude:c1", "completed"], ["claude:0123456789", "completed"]]);
});
