...
test("waits for the source reply before sending the title prompt", async () => {
  ...
  // pi-agent-core >= 0.87 carries the system prompt as a leading system message.
...
test("falls back to the session's effective prompt when the transcript has no system message yet", () => {
  const source = {
    state: { systemPrompt: "", model: { provider: "test", id: "m" }, thinkingLevel: "off", tools: [], messages: [] },
  };
  assert.equal(buildSessionTitleAgentOptions(source, "effective prompt").initialState.systemPrompt, "effective prompt");
  const withTranscriptPrompt = { ...source, state: { ...source.state, systemPrompt: "from transcript" } };
  assert.equal(buildSessionTitleAgentOptions(withTranscriptPrompt, "effective prompt").initialState.systemPrompt, "from transcript");
});
