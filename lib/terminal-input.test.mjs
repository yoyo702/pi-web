...
test("reserves platform copy shortcuts only when terminal text is selected", async () => {
  const { isTerminalCopyShortcut } = await loadSubject();

  assert.equal(isTerminalCopyShortcut(key("c", { metaKey: true }), true), true);
  assert.equal(isTerminalCopyShortcut(key("c", { ctrlKey: true, shiftKey: true }), true), true);
  assert.equal(isTerminalCopyShortcut(key("c", { ctrlKey: true }), true), false);
  assert.equal(isTerminalCopyShortcut(key("c", { metaKey: true }), false), false);
});
...
test("applies one-shot mobile modifiers", async () => {
  const { applyTerminalModifier } = await loadSubject();
  assert.equal(applyTerminalModifier("ctrl", "c"), "\x03");
  assert.equal(applyTerminalModifier("ctrl", "?"), "\x7f");
  assert.equal(applyTerminalModifier("alt", "x"), "\x1bx");
});

test("limits terminal height when a mobile keyboard changes the visual viewport", async () => {
  const { getTerminalVisibleHeight } = await loadSubject();
  assert.equal(getTerminalVisibleHeight(800, 500, 0, 100), 400);
  assert.equal(getTerminalVisibleHeight(800, 760, 0, 100), null);
});
