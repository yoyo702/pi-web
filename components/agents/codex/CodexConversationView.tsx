...
export function CodexConversationView({ items, cwd }: { items: CodexConversationItem[]; cwd: string }) {
  return <>{items.map((item) => {
    ...
    if (item.kind === "todo") return <section key={item.id} style={cardStyle}><div style={labelStyle}>Tasks</div>{item.steps.map((step, index) => <div key={index}>{step.status === "completed" ? "●" : step.status === "inProgress" ? "◐" : "○"} {step.text}</div>)}</section>;
