...
function DraftComposer({ draftKey, running, sentHistory, modelLabel, modelValue, modelOptions, modelMenuRequest, reasoningEffort, serviceTier, approvalPolicy, approvalLabel, statusLabel, activityLabel, noticeLabel, contextLabel, expandDetails, forkDisabled, onModelToggle, onModelChange, onReasoningEffortChange, onServiceTierChange, onApprovalPolicyChange, onExpandDetails, onFork, onSend, onSteer, onQueue, onStop }: { draftKey: string; running: boolean; sentHistory: string[]; modelLabel: string; modelValue: string; modelOptions: ModelOption[]; modelMenuRequest: number; reasoningEffort: string; serviceTier: string; approvalPolicy: "untrusted" | "on-request" | "never"; approvalLabel: string; statusLabel: string; activityLabel?: string; noticeLabel?: string; contextLabel?: string; expandDetails: boolean; forkDisabled: boolean; onModelToggle: () => void; onModelChange: (model: string) => void; onReasoningEffortChange: (effort: string) => void; onServiceTierChange: (tier: string) => void; onApprovalPolicyChange: (policy: "untrusted" | "on-request" | "never") => void; onExpandDetails: () => void; onFork: () => void; onSend: (text: string, images?: ChatDraftImage[]) => Promise<boolean>; onSteer: (text: string, images: ChatDraftImage[]) => Promise<"steered" | "queue" | "failed">; onQueue: (message: { text: string; images: ChatDraftImage[] }) => void; onStop: () => Promise<void> }) {
  ...
  const submit = useCallback(async (mode: "send" | "queue" = "send") => {
    ...
    if (running) {
      ...
      const result = mode === "queue" || text.startsWith("/") ? "queue" : await onSteer(text, images);
    ...
    setHistoryIndex(-1);
  }, [draftKey, images, onQueue, onSend, onSteer, running, updateDraft, value]);
