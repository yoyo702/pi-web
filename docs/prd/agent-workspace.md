...
- 接口（所有 POST 请求体都带 `cwd`；cwd 须是已授权目录，会话须属于该目录）：
  ...
  - `GET /api/claude/chat/commands?cwd=`：返回 `{commands: [{name, description, argumentHint, aliases?}]}`，供输入框的斜杠命令菜单使用。服务端不带会话参数启动一次 `claude --print`，只发 `initialize` 控制请求，读出答复里的 `commands`（包括技能、插件命令）后结束进程；这样不建会话文件，约 0.2 秒。结果按目录缓存 5 分钟（最多 50 个目录）；Claude 列不出（未安装、超时 15 秒、出错）时返回空列表，服务端日志记一条警告，1 分钟内不再重试；答复里没有 `commands` 时保留原来的列表；同名命令只保留第一个。每次启动聊天进程时也发一次 `initialize`，用答复更新缓存，并推送 `pi/commands {commands}` 事件。过滤掉：`__` 开头的内部命令、只能在终端里用的命令（`system/init` 的 `terminal_slash_commands`，如 `/doctor`）、以及 `/clear` 和它的别名 `/reset`、`/new`。
  - `GET /api/claude/chat/:id/agents/:toolUseId?cwd=`：子代理的步骤。Claude 把子代理记录存在 `<会话 id>/subagents/agent-<agentId>.jsonl`，旁边的 `agent-<agentId>.meta.json` 记录它对应的 Agent 工具调用 `toolUseId`；服务端按 `toolUseId` 找到文件，读末尾 4 MiB，返回 `{records, truncated}`（记录带 `parentToolUseId`，裁剪规则同历史）。`toolUseId` 格式不对 400；找不到（旧版 Claude 没存、或 Fork 出来的会话——Fork 不复制 `subagents` 目录）404 “This agent's steps were not saved”；会话文件还没写出时返回空列表。
...
- 事件：转发 Claude 的 stream-json 记录（`stream_event`、`assistant`、`user`、`result`、`control_request`；`system` 只保留 init/status 中的模型和权限模式、压缩状态 `{subtype:"status", compacting}`（只在权限模式或压缩状态变化时发，Claude 每次请求都会发的其他状态丢弃）、`compact_boundary`，以及子代理进度 `task_started` / `task_progress` / `task_notification`（`{tool_use_id, description, last_tool_name, status, usage:{tool_uses, duration_ms}}`）），每条加 `piSeq`、`piRuntime`，`assistant` 记录加 `piBlockIndex`（对应流式事件的内容块序号，用于把流式文本替换成最终文本）。子代理的记录带 `parentToolUseId`（Claude 的 `parent_tool_use_id`，即启动它的 Agent 工具调用），不加 `piBlockIndex`；子代理的 `stream_event` 丢弃。Claude 为自己写的压缩摘要（`isSynthetic` / `isCompactSummary`）不转发、历史里也不返回。另有服务端事件：`pi/commands`（见上）、`pi/resolved`（请求已回答，或 Claude 发来 `control_cancel_request` 取消了它）、`pi/closed`（进程意外退出，附原因；Claude 的 `system/init` 报告的会话 id 与本聊天不同——某个命令像 `/clear` 那样换了会话——时停止进程，`code: "session_changed"`，显示 “Claude moved to another session. Start a new chat to continue.”）、`pi/stopped`（进程被主动停止）。一轮结束（`result`）后，缓冲中的 `stream_event` 被丢弃，只保留完整记录。前端的运行状态只跟随事件（`user` → 运行中，`result`/`pi/closed`/`pi/stopped` → 空闲），不因 POST 成功而设置。
- 聊天界面（Claude Chat 标签，复用 Codex 聊天的消息列表和输入框）：
  ...
  - 斜杠命令菜单：输入框以 “/” 开头且没有空格时弹出，列出 Claude 的命令（上面的 `commands` 接口，打开聊天时读取，进程启动后以 `pi/commands` 更新）。按命令名、别名或插件前缀 “:” 之后的部分匹配前缀；每项显示 `/名称`、参数提示（灰色）和说明，列表可滚动，↑/↓ 选择，Enter/Tab 填入（有参数提示时末尾加空格），Esc 关闭。命令作为普通消息发送，由 Claude 执行。
  - `/clear`（及 `/reset`、`/new`）被拒绝（400 “/clear would move this chat to a new Claude session. Start a new chat instead.”）：Claude 执行它会换一个新会话 id，聊天就跟丢了会话。要清空请新开聊天。
  - `/context` 等本地命令：Claude 实时发来一条合成的 `assistant` 消息（`model: "<synthetic>"`），会话文件里存成同一 uuid 的 `system/local_command` 记录；历史读取把它转换成同样的 `assistant` 记录（`message.id` 取 uuid），所以实时和重新加载显示一致。
  - `/compact`（以及自动压缩）：压缩期间输入框上方显示 “Compacting the conversation…”，完成后在该位置显示提示 “Conversation compacted”（来自 `compact_boundary`）；Claude 随后回放的 “Compacted” 命令输出不再单独显示，摘要内容不显示。
  - 子代理：Agent（旧版 Task）工具调用下有 “Agent steps” 折叠行，运行中显示最近的工具和工具调用次数（来自 `task_progress`）。展开时读取子代理步骤接口，与本次运行中实时收到的子代理记录按 uuid 合并；子代理结束时再读取一次。显示子代理的文字、命令、工具调用（嵌套的 Agent 调用可以继续展开），不显示它的提示词（就是工具输入）。没有保存也没有实时记录时显示 “Steps are not available for this agent.”。主对话里不显示子代理记录。
